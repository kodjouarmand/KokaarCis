...
{
    public class InvoiceHeaderController : BaseUserController
    {
        ...
        public IActionResult AddOrEdit(InvoiceHeaderViewModel invoiceHeaderViewModel)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (invoiceHeaderDto.Id == 0)
                    {
                        ...
                        invoiceHeaderViewModel = GetInvoiceHeaderViewModel(invoiceHeaderDto.Id, true);
                        return Json(new
                        {
                            isValid = true,
                            message = "Opération effectuée avec succès.",
                            html = MvcHelper.RenderRazorViewToString(this, "Print", invoiceHeaderViewModel)
                        });
                    }
                    else
                    {
                        ...
                        string returnHtml;
                        if (invoiceHeaderViewModel.ReturnToHeaderView)
                        {
                            invoiceHeaderViewModel = GetInvoiceHeaderViewModel(invoiceHeaderDto.Id, loadDetail: true,
                                returnToHeaderView: invoiceHeaderViewModel.ReturnToHeaderView);
                            returnHtml = MvcHelper.RenderRazorViewToString(this, "_Header", invoiceHeaderViewModel);
                        }
                        else
                        {
                            var invoiceHeaderDtos = _invoiceHeaderQuery.GetAll().AsQueryable().GetPaged(1, ConstantHelper.DEFAULT_PAGE_SIZE);
                            returnHtml = MvcHelper.RenderRazorViewToString(this, "_List", invoiceHeaderDtos);
                        }
                        return Json(new
                        {
                            isValid = true,
            ...
            catch (Exception ex)
            {
                ...
                return Json(new
                {
                    isValid = false,
                    message = ex.Message,
                    html = MvcHelper.RenderRazorViewToString(this, "AddOrEdit", invoiceHeaderViewModel)
                });
            }
        }
